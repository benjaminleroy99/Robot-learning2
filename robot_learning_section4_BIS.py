...
class Agent:
    ...
    def train_neural_net(self,network,optimiser):
        ...
        for training_iteration in range(20): # esssayer 500
...
class MainProgram(arcade.Window):
    ...
    def __init__(self):
        ...
        nb_demonstrations=30


        #we first create 100 demonstrations to create a dataset
        for i in tqdm(range(nb_demonstrations)):

            self.initial_state=np.random.randint(0,4,2)/10

            self.agent.robot.state=self.initial_state

            best_seq_actions=self.demonstrator.compute_actions(initial_state_origin=self.initial_state,num_sequences=self.num_sequences, num_actions_per_sequence=self.num_actions_per_sequence,environment=self.environment)
        ...
        optimiser = torch.optim.Adam(network.parameters(), lr=0.0005)

        nb_iterations=101
        lists_when_interesting_positions=[0,1,5,20,100]
        self.seqs_positions=[]

        for nb_it in tqdm(range(nb_iterations)):
            ...
            if nb_it!=0:
                ...
                self.agent.train_neural_net(network, optimiser)

            for i_act in range(self.nb_actions):
            ...
            if nb_it in lists_when_interesting_positions:
                self.seqs_positions.append(positions)

            for position in positions:

                min_distance=100000
                potential_action=None
                for demo_i in dataset_moves:
                    distance=np.linalg.norm(position-demo_i[0])
                    if distance<=min_distance:
                        potential_action=demo_i[1]
                        min_distance=distance

                dataset_moves.append(np.array([position,potential_action]))
    ...
    def on_draw(self):
        ...
        for positions in self.seqs_positions:
