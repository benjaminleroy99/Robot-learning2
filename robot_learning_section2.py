...
class Agent:
    ...
    def train_neural_net(self):
        ...
        for training_iteration in tqdm(range(50)): # avant 750
...
class Demonstrator:
    ...
    def __init__(self,agent):
        ...
        self.waypoints = [np.array([0.2, 0.55]), np.array([0.45, 0.82]), np.array([0.85, 0.8]), np.array([0.65, 0.5]),
                          np.array([0.65, 0.15])]
...
class MainProgram(arcade.Window):
    ...
    def __init__(self):
        ...
        self.initial_state=np.array([0.2, 0.2])
        self.agent.robot.state=self.initial_state

        self.best_seq_actions=self.demonstrator.compute_actions(initial_state_origin=self.initial_state,num_sequences=self.num_sequences, num_actions_per_sequence=self.num_actions_per_sequence,
                                                         environment=self.environment)

        self.nb_actions = len(self.best_seq_actions)

        print("self.best_seq_actions")
        print(self.best_seq_actions)
        self.network=self.agent.train_neural_net()
    ...
    def on_draw(self):
        ...
        for i in range(self.nb_actions):
            ...
            action_i = self.network.forward(position).detach().numpy()
            print("action_i")
            print(action_i)
        ...
        print("positions provided by network")
        print(positions)
        ...
        for i in range(self.nb_actions):
            ...
            elif i==self.num_actions_per_sequence-1:
                arcade.draw_circle_filled(pos[0],pos[1],radius=5, color=[0, 255, 255])

            else:
                arcade.draw_circle_filled(pos[0],pos[1],radius=5, color=[220,220,220])

        arcade.draw_line_strip(point_list=scaled_positions,color=[220,220,220],line_width=3)
