...
class Agent:
    ...
    def train_neural_net(self,batch_size):
        ...
        nb_demo = len(dataset_moves)

        if nb_demo<5:

            dataset=[]
            for demo in dataset_moves:
                for move in demo:
                    dataset.append(move)

            nb_moves=len(dataset)
            train_set = np.array(dataset[:int(0.8 * nb_moves)])

            test_set = np.array(dataset[int(0.8 * nb_moves):])



        else:#we can take 20%

            train_set_demos = np.array(dataset_moves[:int(0.8 * nb_demo)])
            train_set=[]
            for demo in train_set_demos:
                for move in demo:
                    train_set.append(move)

            test_set_demos = np.array(dataset_moves[int(0.8 * nb_demo):])

            test_set=[]
            for demo in test_set_demos:
                for move in demo:
                    test_set.append(move)
            '''
            print("test_set")
            print(test_set)
            print("train_set")
        ...
        optimiser = torch.optim.Adam(network.parameters(), lr=0.001)  #0.005
        ...
        for training_iteration in range(200): # avant 750
...
class Demonstrator:
    ...
    def compute_actions(self, initial_state_origin, num_sequences, num_actions_per_sequence, environment):
        ...
        for i in range(nb_actions):
            ...
            demonstration_dataset.append([old_position, action_i])

        dataset_moves.append(demonstration_dataset)
